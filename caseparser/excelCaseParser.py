...
# sys.path.append("/opt/17173/sourcecode/python/chris/cyouapitestframework")
...
class ExcelCaseParser(IParser):
    ...
    def _open_sheet(self, work_book):
        return self.open_sheet_by_name(work_book, "TestCases")
    ...
    def open_sheet_by_name(self, work_book, sheet_name):
        ...
        for st_name in work_book.sheet_names():
            if str(st_name) == sheet_name:
                _sheet = work_book.sheet_by_name(st_name)
    ...
    def _parse_assert_pattern_field(self, assert_pattern):
        # _parse_assert_pattern = self._parse_test_content_from_field(assert_pattern)
        # return self._parse_multiline_test_data(_parse_assert_pattern)
        _return_field = self._parse_test_content_from_field(assert_pattern)

        return self._parse_multiline_test_data(_return_field)

    def _parse_assert_value_field(self, assert_value):
        # _parse_assert_value = self._parse_test_content_from_field(assert_value)
        # return self._parse_multiline_test_data(_parse_assert_value)

        if isinstance(assert_value, str) or isinstance(assert_value, unicode):
            _return_field = self._parse_test_content_from_field(assert_value)
        else:
            _return_field = assert_value
    ...
    def _parse_multiline_test_data(self, multiline_test_data):
        if isinstance(multiline_test_data, int) or isinstance(multiline_test_data, float) or \
                isinstance(multiline_test_data, long):
            if type(multiline_test_data) == float:
                if multiline_test_data == int(multiline_test_data):
                    multiline_test_data = int(multiline_test_data)

            multiline_test_data = unicode(multiline_test_data)

        if isinstance(multiline_test_data, unicode) and multiline_test_data != "":
            if multiline_test_data.find("\n") >= 0:
        ...
        for _result_item in _result_list:
            if _result_item == "":
                _result_list.remove(_result_item)
...
if __name__ == "__main__":
    ...
    _excel_parser = ExcelCaseParser("Z:\\cyouapitestframework\\testcases\\temptestcase.xls")
